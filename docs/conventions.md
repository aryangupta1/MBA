...
### Live-session pages (gated)

```
live/<CODE>.html            per-subject index
...
| A live-session note | `live/`, built by `.claude/private-pages/build.py` with the access-code gate — never as plaintext |
