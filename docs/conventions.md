...
### Live-session and assessment pages (gated)

```
live/<CODE>.html                per-subject index (weeks, then assessment notebooks)
live/<CODE>-week<N>.html        one per week
live/<CODE>-assessment<N>.html  one per assessment notebook, discovered from the vault
```

Generated by `.claude/private-pages/build.py`, never hand-written. Content is encrypted in
the file under the access code in `.env`; the pages carry `noindex` and `robots.txt` covers
the directory, but unlike `only-accessible-by-url/` they **are** linked from the hubs and
`library.html`. `<N>` in an assessment filename is the number in the vault folder's
`Assessment <N> …` name.
...
| A live-session note or assessment notebook | `live/`, built by `.claude/private-pages/build.py` with the access-code gate — never as plaintext, never on a public page |
