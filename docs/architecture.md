...
│
├── DMBA6008-weeks.html        Semester 2 — GENERATED by the sync-subject skill from the
├── DMBA6008-week0..6.html     Obsidian vault (DESK design system, one file each)
├── DMBA6008-search.html       Master search page, derived from the week pages above
├── DMBA6005-weeks.html        Same family for DMBA 6005
├── DMBA6005-week0..6.html
├── DMBA6005-search.html
│
...
│   ├── power-interest.html
│   ├── metadata.html
│   └── SECRET-PAGES.md        Index of the live/ pages (kept here so robots.txt covers it)
│
...
- **The Semester 2 pages are this family, but generated.** `DMBA6008-*.html` and
  `DMBA6005-*.html` are built from the Obsidian vault by the `sync-subject` skill and share
  the DESK design system ([design-system.md](design-system.md)); each subject's
  `-search.html` is derived one step further, from the built week pages, by
  `reference/search/build_search.py`. Edit the source or the template and regenerate —
...
- `robots.txt` covers both `/MBA/only-accessible-by-url/` and `/only-accessible-by-url/`,
  and likewise `/MBA/live/` and `/live/`, so the rules hold whether the site is served from
  a subpath or a domain root.
- **`live/` is gated, not hidden.** Its pages are linked from the hubs and the library; what
  protects them is that the content is ciphertext until the reader supplies the access code.
  `.env` holds the code and is git-ignored — a commit that includes it defeats the whole
  arrangement.
